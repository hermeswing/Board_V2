...
public class KaptchaServlet {
	...
	// ajax
	@RequestMapping(value = "/kaptcha.do", method = RequestMethod.POST)
	protected ModelAndView validateCaptcha(ModelAndView mav, HttpSession session,
			@RequestParam(value = "kaptcha") String input) throws Exception {
		mav.setViewName("ajax/auth");
		
		String kaptcha = (String) session.getAttribute(Constants.KAPTCHA_SESSION_KEY);

		mav.addObject("auth", kaptcha.equals(input));
